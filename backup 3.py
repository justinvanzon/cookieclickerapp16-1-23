...
class App:
    ...
    def on_loop(self):
        ...
        for mouse_pos in positions:
            screen.blit(click_text, (mouse_pos[0], mouse_pos[1] - 15))
        pygame.display.flip()

    def on_event(self, event):
        global radius, rect, Cookies, Cookies_PC, mouse_pos, Price1, Cookies_CPS, click_text, positions, MouseClick, X, Z
        if event.type == pygame.QUIT:
            self._running = False
        if event.type == pygame.USEREVENT:
            Cookies += (Cookies_CPS / 40)
        if event.type == pygame.USEREVENT + 1 and len(positions) > 0:
