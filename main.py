...
class App:
    ...
    def on_event(self, event):
        ...
        if event.type == pygame.USEREVENT + 1 and len(positions) == 1:
            positions.remove(mouse_pos)
        if event.type == pygame.USEREVENT + 1 and len(positions) > 1:
            positions.remove(mouse_pos)
        if event.type == pygame.USEREVENT + 2 and len(positions) > 2:
            positions.remove(mouse_pos)
        #if event.type == pygame.USEREVENT + 3:
            #new_positions = [(mouse_pos[0], mouse_pos[1]) for (mouse_pos[0], mouse_pos[1] - 5) in positions]
